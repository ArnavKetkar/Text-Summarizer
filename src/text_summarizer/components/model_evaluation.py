...
class ModelEvaluation:
    ...
    def generate_batch_sized_chunk(self, list_of_elements, batch_size):
        """
        Split dataset into smaller batches that can be processed simulataneously
        Yield successive batch sized chinks from list of elements
        """
    ...
    def calculate_metric_on_test_ds(self,
        ...
        for article_batch, target_batch in tqdm(zip(article_batches, target_batches), total=len(article_batches)):
            ...
            # Now we decode the generated texts
            # replace the  token, and add the decoded texts with the references to the metric.
            
            decoded_summaries = [tokenizer.decode(s, skip_special_tokens=True, clean_up_tokenization_spaces=True) for s in summaries]
            
            decoded_summaries = [ds.replace("", " ") for ds in decoded_summaries]
